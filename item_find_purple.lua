...
-- 클릭 객체
	...
	local S1=display.newRect(display.contentWidth*0.333, display.contentHeight*0.1,90,75)
	S1:setFillColor(0)
	sceneGroup:insert(S1)

-- 배경 객체
	local background = display.newImageRect("이미지/대화/우정배경/보라.png",display.contentWidth, display.contentHeight)
	...
	local pic = display.newImageRect("이미지/숨은그림찾기/숨은그림찾기-글러브.png",display.contentWidth*0.5, display.contentHeight*0.9)
	pic.x,pic.y = display.contentWidth*0.5,display.contentHeight*0.5
	sceneGroup:insert(pic)

	local mic = display.newImage("이미지/숨은그림찾기/정답-글러브.png")
	mic.x,mic.y = display.contentWidth*0.3333,display.contentHeight*0.1035
	mic.alpha=0
...
-- 성공 객체
	...
	success.x,success.y = display.contentWidth*0.333,display.contentHeight*0.1
	success.alpha=0
