-- item_find_color.lua
-- 숨은그림찾기 공통 모듈
-- composer.setVariable("find_config", { colorId, sceneName, bgImage, picImage, answerImage, s1x, s1y, s1w, s1h, s1Color, micX, micY })
-- 으로 설정 후 gotoScene("item_find_color") 호출
...
function scene:create( event )
    ...
    local config = composer.getVariable("find_config")
...
function scene:hide( event )
    if event.phase == "did" then
        local config = composer.getVariable("find_config")
        if config then
            composer.removeScene(config.sceneName)
        end
    end
end
