...
function scene:create( event )
	...
	composer.setVariable( "color", 3 )

-- 클릭 객체

	local S1=display.newRect(display.contentWidth*0.265, display.contentHeight*0.85,70,200)
	S1:setFillColor(0)
	sceneGroup:insert(S1)

-- 배경 객체
	local background = display.newImageRect("이미지/대화/우정배경/노랑.png",display.contentWidth, display.contentHeight)
	...
	local pic = display.newImageRect("이미지/숨은그림찾기/숨은그림찾기-마이크.png",display.contentWidth*0.5, display.contentHeight*0.9)
	pic.x,pic.y = display.contentWidth*0.5,display.contentHeight*0.5
	sceneGroup:insert(pic)

	local mic = display.newImage("이미지/숨은그림찾기/정답-마이크.png")
	mic.x,mic.y = display.contentWidth*0.2615,display.contentHeight*0.8655
	mic.alpha=0
...
-- 성공 객체
	...
	success.x,success.y = display.contentWidth*0.26,display.contentHeight*0.85
	success.alpha=0
...
-- 시간 객체
	...
	local function find(event)
		...
		timer.cancelAll() 
		composer.setVariable("color", 3)      
		composer.removeScene( "item_find_yellow" )
      local options={
			effect ="fade",
			time=1000
		}
