...
function scene:create( event )
	...
	composer.setVariable( "color", 1 )

-- 클릭 객체

	local S1=display.newRect(display.contentWidth*0.585, display.contentHeight*0.6,70,75)
	S1:setFillColor(1)
	sceneGroup:insert(S1)

-- 배경 객체
	local background = display.newImageRect("이미지/대화/우정배경/빨강.png",display.contentWidth, display.contentHeight)
	...
	local pic = display.newImageRect("이미지/숨은그림찾기/숨은그림찾기-하트수첩.png",display.contentWidth*0.5, display.contentHeight*0.9)
	pic.x,pic.y = display.contentWidth*0.5,display.contentHeight*0.5
	sceneGroup:insert(pic)

	local mic = display.newImage("이미지/숨은그림찾기/정답-하트수첩.png")
	mic.x,mic.y = display.contentWidth*0.5865,display.contentHeight*0.597
	mic.alpha=0
...
-- 성공 객체
	...
	success.x,success.y = display.contentWidth*0.5865,display.contentHeight*0.597
	success.alpha=0
...
-- 시간 객체
	...
	local function find(event)
		...
		timer.cancelAll()
		composer.setVariable("color", 1)      
		composer.removeScene( "item_find_red" )
      local options={
			effect ="fade",
			time=1000
		}
